...
if __name__ == "__main__":
    ...
    bag_path = '/root/catkin_ws/src/om_position_controller/recording/pick.bag'
    ...
    dmp_path = '/root/catkin_ws/src/om_position_controller/recording/pick_motion.pkl'
    ...
    new_goal[:3] = np.array(trans_3) + np.array([+0.0, 0.0, +0.01])
    ...
    try:
        ...
        trajectory_execution_time = max(interpolated_time) * 5
        print(f"[Gazebo] Waiting {trajectory_execution_time:.2f} seconds for trajectory completion...")
        rospy.sleep(trajectory_execution_time + 2.0)
        
        print("[Gazebo] Trajectory execution completed. Returning to home position...")
        publisher.publish_home_position(
            home_position=[-0.03834952, -0.84062147, 1.26093221, 0.00613592, 1.97576725, -0.00460194],
            execution_time=5.0
        )
        
        print("[Gazebo] Home position command published and latched for 3.0 seconds")
        rospy.sleep(3.0)
